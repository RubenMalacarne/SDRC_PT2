...
namespace coppelia_pick_and_place
{
    ...
    {
    public:
        explicit MotionActionServer(const rclcpp::NodeOptions &options = rclcpp::NodeOptions()) : Node("motion_action_server", options)
        {
            ...
            action_server_ = rclcpp_action::create_server<coppelia_msgs::action::CoppeliaTask>(
                this, "motion_action_server",
                std::bind(&MotionActionServer::goalCallback, this, _1, _2),
                std::bind(&MotionActionServer::cancelCallback, this, _1),
                std::bind(&MotionActionServer::acceptedCallback, this, _1));
        }

    private:
        const float GRASP_OFFSET = 0.30; // [m]
        double arr[3] = { 0.9, 0.625, 0.939 }; 
            ...
            const std::shared_ptr<rclcpp_action::ServerGoalHandle<coppelia_msgs::action::CoppeliaTask>> goal_handle)
        {
            ...
            std::thread{std::bind(&MotionActionServer::execute, this, goal_handle)}.detach();
        }
        ...
        void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<coppelia_msgs::action::CoppeliaTask>> goal_handle)
        {
            ...
            {
                // Pre Grasp
                // 1. Accertasi di essere in home - Move up by an offset
                geometry_msgs::msg::Pose current_pose = move_group.getCurrentPose().pose;
                std::vector<geometry_msgs::msg::Pose> move_up_waypoints;
                geometry_msgs::msg::Pose pose_up  = current_pose;
                pose_up.position.z -= GRASP_OFFSET;
                move_up_waypoints.push_back(pose_up);

                moveit_msgs::msg::RobotTrajectory trajectory_move_it;
                const double jump_threshold = 0.0;
                const double eef_step = 0.01;
                ...
                if (fraction > 0.99)
                {
                    move_group.execute(trajectory_move_it);
                    std::this_thread::sleep_for(std::chrono::seconds(1));
                }
                else
                {
                    RCLCPP_WARN(LOGGER, "Cartesian path planning incomplete. Skipping execution.");
                }
...
RCLCPP_COMPONENTS_REGISTER_NODE(coppelia_pick_and_place::MotionActionServer)